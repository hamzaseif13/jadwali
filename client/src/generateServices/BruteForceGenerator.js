import { fetchSections } from "../context/jadwaliContext/JadwaliActions";
...
export class Generator {
     ...
     */
    checkTimeValidation(schedule) {
        let valid = true;
        schedule.forEach((section) => {
            if (section.startTime < this.options.startTime)
                valid = false;
            if (section.endTime > this.options.endTime)
                valid = false;
        });
        return valid;
    }
    ...
    */
    static fetchSections(courses) {
        return Promise.all(courses.map((lineNumber) => {
            return fetchSections(lineNumber); 
        }));
    }
    /**
    ...
    checkForConflicts(schedule) {
        ...
        let days = [[], [], [], [], []];
        schedule.forEach((section) => {
            if (section.days.includes("sun"))
                days[0].push(section);
            if (section.days.includes("mon"))
                days[1].push(section);
            if (section.days.includes("tue"))
                days[2].push(section);
            if (section.days.includes("wed"))
                days[3].push(section);
            if (section.days.includes("thu"))
                days[4].push(section);
        });
        days.forEach((day) => {
            if (day.length > 1) {
                for (let j = 0; j < day.length - 1; j++)
                    for (let i = j + 1; i < day.length; i++)
                        if (this.checkInterSection(day[j], day[i]))
                            noConflict = false;
            }
        });
        return noConflict;
    }
    checkIfActiveOrFull(schedule) {
        let validSchedule = true;
        schedule.forEach((section) => {
            if (section.status.toLowerCase() == "cancelled")
                validSchedule = false;
        });
        return validSchedule;
    }
    checkIfMeetsPinned(schedule) {
        const pinnedIds = this.options.pinnedSections.map((section) => section.id);
        return pinnedIds.every((id) => schedule.map((section) => section.id).includes(id));
    }
    scheduleIsValid(schedule) {
        return (this.checkDaysValidation(schedule) &&
     ...
     */
    checkDaysValidation(schedule) {
        let valid = true;
        schedule.forEach((section) => {
            let days = section.days.split(" ");
            days.forEach((day) => {
                if (!this.options.days.includes(day))
                    valid = false;
            });
        });
        return valid;
    }
    sortByScore(schedules) {
        schedules.sort((a, b) => {
...
export class BruteForceGenerator extends Generator {
     ...
     */
    getAllCombinations(sections) {
        const result = [];
        const backtrack = (i, state) => {
    ...
    generate(sections) {
        ...
        return filteredSchedules.slice(0, 1000);
    }
}
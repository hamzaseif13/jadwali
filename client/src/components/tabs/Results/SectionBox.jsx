...
function SectionBox({ section, index }) {
  const { registeredCourses } = useContext(JadwaliContext);
  let { startTime, endTime } = section;
  return (
    <div
      className={`absolute start${startTime * 10
        }  w-full bg-gray-500 border-l-8 border rounded-t rounded-l  height${(endTime - startTime) * 10
        } text-left px-1 overflow-clip text-ellipsis md:text-base ${registeredCourses[index].color
        } border-y-mydark border-r-mydark `}>
      <h1 className="text-xs">
        {registeredCourses[index].name} {section.number}
      </h1>
      <h1 className="text-[0.6rem] sm:text-xs text-center">
